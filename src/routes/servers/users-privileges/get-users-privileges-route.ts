...
export const getUsersPrivilegesRoute = async (app: FastifyInstance) => {
    app.withTypeProvider<ZodTypeProvider>().post(
        ...
        async (request, reply) => {
            ...
            const privilegesActives = await prisma.privileges.findMany({
                where: {
                    dt_expires_at: {
                        gt: new Date(),
                    },
            ...
            const responses = await Promise.all(
                servers.map(async (server) => {
                    ...
                    const users = data.result.map((user) => {
                        const privilege = privilegesActives.find(
                            (item) =>
                                item.ds_user === user.username &&
                                item.ds_server === server
                        );

                        return {
                            ...user,
                            privilegeActive: !!privilege,
                            privilege,
                        };
                    });
